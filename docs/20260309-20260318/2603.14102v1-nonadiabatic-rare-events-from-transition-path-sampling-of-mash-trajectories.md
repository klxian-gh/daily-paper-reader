...
title_zh: 基于 MASH 轨迹过渡路径采样的非绝热稀有事件
authors: "Danial Ghamari, Jeremy O. Richardson"
date: 2026-03-14
pdf: "https://arxiv.org/pdf/2603.14102v1"
tags: ["query:qmmm"]
score: 7.0
evidence: 模拟稀有非绝热反应机理和路径
...
motivation: 稀有非绝热反应在分子过程中至关重要，但由于其发生频率低，传统的直接动力学模拟难以有效捕获。
method: 将具有时间可逆性和马尔可夫特性的映射面跳跃法（MASH）与过渡路径采样（TPS）技术相结合。
result: 在宽范围的二元耦合强度下，成功模拟了自旋-玻色子模型的反应路径并准确获取了动力学性质。
conclusion: 该框架为研究超越暴力模拟能力的稀有非绝热事件提供了一种实用且高效的系统化方案。
---

## 摘要
稀有非绝热反应是许多重要分子过程的关键组成部分，但通过直接动力学模拟很难捕捉。在本文中，我们将最近开发的映射面跳跃方法 (MASH) 与过渡路径采样相结合，构建了一个高效模拟这些稀有事件的框架。这得益于 MASH 轨迹具有马尔可夫性、时间可逆性并遵循刘维尔定理。该结合方法在不偏离底层动力学的情况下生成非绝热反应路径。所得系综允许对反应机制进行详细分析，并揭示统计和动力学性质（包括速率常数）。我们将该方法应用于研究热平衡下具有广泛非绝热耦合强度的自旋-玻色子模型。结果表明，该方法为研究稀有非绝热过程提供了一个实用且系统的工具，其能力可能超出了暴力模拟的范围。
