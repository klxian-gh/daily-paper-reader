...
tags: ["query:qmmm"]
score: 7.0
evidence: 原子级建模与模拟工作流的自动化
tldr: 针对大语言模型在高性能计算（HPC）原子模拟中存在的正确性和安全性挑战，本文提出了LARA-HPC框架。该框架采用验证驱动的智能体方法，通过受控执行层、无成本的模拟预检（dry-run）以及结合检索增强生成与迭代优化的多阶段流水线，实现了可靠的原子建模工作流生成。在密度泛函理论（DFT）模拟中的应用证明，该方法能显著提升工作流的鲁棒性，并有效修正语法和物理配置错误，为AI辅助的科学计算提供了新范式。
selection_source: fresh_fetch
motivation: 现有的LLM生成工作流在HPC环境下常因API误用或物理配置无效而导致模拟失败，缺乏确保正确性和安全性的机制。
method: 提出了包含受控执行层、模拟原生验证（dry-run）和多阶段迭代优化流水线的LARA-HPC框架。
result: 在DFT模拟任务中，该框架显著提高了工作流生成的鲁棒性，并能自动识别并修复语法及物理层面的不一致性。
conclusion: 研究倡导从“生成优先”转向“验证优先”的AI辅助科学计算范式，强调构建基于结构化工具的领域特定智能体系统的重要性。
---
