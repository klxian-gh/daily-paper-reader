...
title_zh: 通过 OpenClaw 和领域特定技能实现计算化学工作流的自动化
...
## 摘要
自动化多步计算化学任务仍然具有挑战性，因为推理、工作流规范、软件执行和高性能计算（HPC）执行通常是紧密耦合的。我们展示了一种利用 OpenClaw 实现计算化学自动化的解耦智能体-技能（agent-skill）设计。具体而言，OpenClaw 提供集中控制和监督；模式定义（schema-defined）的规划技能将科学目标转化为可执行的任务规范；领域技能封装了特定的计算化学程序；而 DPDispatcher 负责管理异构 HPC 环境中的作业执行。在甲烷氧化的分子动力学（MD）案例研究中，该系统完成了跨工具执行、运行时故障的有界恢复以及反应网络提取，展示了一种可扩展且易于维护的多步计算化学自动化方法。
