...
date: 2026-05-11
pdf: "https://arxiv.org/pdf/2605.10363v1"
tags: ["query:qmmm"]
score: 6.0
evidence: 加速量子化学中的积分计算
tldr: 针对量子化学中具有空间局域性的积分计算（如EXC积分），本文提出了KerneLDI框架。该框架通过协同设计数据布局、筛选逻辑和矩阵运算算子，实现了块结构矩阵乘法。它将操作数矩阵重组为统一的块过滤表示，并利用定制的稠密块乘法器在GPU上高效执行。实验表明，该方法在保持数值精度的同时，显著提升了DFT计算和分子动力学模拟的速度。
selection_source: fresh_fetch
motivation: 传统的稠密或通用稀疏矩阵算子难以高效处理量子化学积分中由空间局域性引起的动态结构化稀疏性。
method: 开发了KerneLDI框架，通过块过滤表示仅保留空间相关的块，并使用自适应的定制稠密块乘法器进行高效收缩计算。
result: 在EXC积分评估中相比稠密GPU基准实现了高达10倍的加速，并在从头算分子动力学中获得了近6倍的吞吐量提升。
conclusion: KerneLDI通过结合空间局域性筛选与高性能GPU算子，为加速大规模量子化学计算提供了一种高效且可扩展的解决方案。
---
